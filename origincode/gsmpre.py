...
# 2. 调试样本数
# 选项: 10 (快速测试), None (跑全量)
# GSM8K 测试集有 1319 条，建议先设为 20-50 条跑通流程
DEBUG_NUM = 200
...
# 4. 数据集配置 (GSM8K 专用配置)
DATASET_NAME = "gsm8k" 
DATASET_CONFIG = "main" # GSM8K 需要指定 config
SPLIT_TRAIN = "train" 
SPLIT_TEST = "test"   
FIELD_MAP = {
    'question': 'question',         
    'answer': 'answer' # GSM8K 的 answer 是一段包含推理和 #### 的长文本
}
...
def main():
    ...
    if MODEL_SOURCE == "gemini":
        ...
        hf_config_dict = {
            ...
            "batch_size":16, # 保守一点，防止显存爆炸或 padding 问题
            "max_input_len": 4096, 
            "max_new_tokens": 1024,
            "save_dir": "rag_result_cache"
        }
