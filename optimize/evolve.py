...
# 提示词生成函数 (复用现有的，或者使用简单的字符串拼接)
from optimize.prompt_generate import apply_gradient_prompt

# ------------------------------------------------------------------------------
# 辅助函数：构造 Student Prompt
# ------------------------------------------------------------------------------

def generate_supplement_prompt(original_content, neg_queries_text, advice):
    """生成补充单一记忆的 Prompt"""
    return f"""
...
def generate_split_prompt(neg_queries_text, advice, num_splits):
    """生成分裂多条记忆的 Prompt"""
    return f"""
You are a Technical Writer.
We need to create {num_splits} distinct memory entries based on specific failed queries.
...
[Task]
Write exactly {num_splits} separate memory entries.
Each entry should be self-contained and accurate.

[Format Requirement]
You MUST separate each memory entry with the delimiter "==========SPLIT==========".
...
def evolve_high_score_opt(cfg: DictConfig, memories: Dict, memory_stats: Dict, high_ids: List[str]) -> Set[str]:
    ...
    for i in range(0, len(target_ids), batch_size):
        ...
        for mid in chunk_ids:
            ...
            # 构造 Prompt
            try:
                prompt = cfg.optimizer.prompts.high_score_evolution.format(
                    content=base_text,
        ...
        for meta, expert_resp in zip(chunk_metadata, expert_outputs):
            ...
            if action == "IGNORE":
                ...
                # 生成单条补充记忆
                s_prompt = generate_supplement_prompt(meta['base_text'], meta['neg_text'], gradient)
                ...
                # 生成多条分裂记忆
                # 限制一下分裂数量，防止 LLM 发疯生成几十条
                safe_split_num = min(max(2, split_num), 5) 
                
                s_prompt = generate_split_prompt(meta['neg_text'], gradient, safe_split_num)
                log_info["student_prompt"] = s_prompt
                log_info["safe_split_num"] = safe_split_num
                
                student_prompts.append(s_prompt)
                student_tasks.append({
                    "parent_mid": mid,
                    "action": "SPLIT",
                    "expected_count": safe_split_num,
                    "log": log_info
                })
        ...
        if student_prompts:
            ...
            for task, output_text in zip(student_tasks, student_outputs):
                log_info = task["log"]
